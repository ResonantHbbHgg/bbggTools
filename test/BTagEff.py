...
process.bbggbtageff.OutFileName = cms.untracked.string(outputFile)
process.bbggbtageff.rho = cms.InputTag('fixedGridRhoAll')
process.bbggbtageff.vertexes = cms.InputTag("offlineSlimmedPrimaryVertices")
process.bbggbtageff.lumiWeight = cms.double(1.0)
...
if options.doSelection is True:
	process.bbggbtageff.doSelectionTree = cms.untracked.uint32(1)
if options.doSelection is False:
	process.bbggbtageff.doSelectionTree = cms.untracked.uint32(0)
if options.doDoubleCountingMitigation is True:
	process.bbggbtageff.doDoubleCountingMitigation = cms.untracked.uint32(1)
	process.bbggbtageff.nPromptPhotons = cms.untracked.uint32(options.nPromptPhotons)
	print "Number of prompt photons in DiPhotonCandidate:",options.nPromptPhotons
if options.doDoubleCountingMitigation is False:
	process.bbggbtageff.doDoubleCountingMitigation = cms.untracked.uint32(0)

#process.p = cms.Path(process.bbggbtageff)


process.p = cms.Path(flashggTags.flashggUnpackedJets*process.bbggbtageff)
