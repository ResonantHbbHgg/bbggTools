#include "flashgg/bbggTools/bbggTools.h"

using namespace std;

double bbggTools::getCHisoToCutValue(edm::Ptr<flashgg::DiPhotonCandidate> dipho, int whichPho, float rho)
{
	if( whichPho > 1 ) {
...
double bbggTools::getNHisoToCutValue(const flashgg::Photon* pho, float rho)
{
...
double bbggTools::getPHisoToCutValue(const flashgg::Photon* pho, float rho)
{
...
bool bbggTools::isPhoISO(edm::Ptr<flashgg::DiPhotonCandidate> dipho, int whichPho, vector<double> cuts)
{
	...
	chiso = bbggTools::getCHisoToCutValue( dipho, whichPho, rhoFixedGrd);
  	if(whichPho == 0) nhiso = bbggTools::getNHisoToCutValue( dipho->leadingPhoton(), rhoFixedGrd );
  	if(whichPho == 1) nhiso = bbggTools::getNHisoToCutValue( dipho->subLeadingPhoton(), rhoFixedGrd );
  	if(whichPho == 0) phiso = bbggTools::getPHisoToCutValue( dipho->leadingPhoton(), rhoFixedGrd );
	if(whichPho == 1) phiso = bbggTools::getPHisoToCutValue( dipho->subLeadingPhoton(), rhoFixedGrd );
