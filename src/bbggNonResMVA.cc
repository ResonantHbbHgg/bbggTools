...
void bbggNonResMVA::SetupNonResMVA(std::string LMFile, std::string HMFile, std::vector<std::string> inVars){
    TMVAReady = 1;
    LowMassReader = new TMVA::Reader();
    HighMassReader = new TMVA::Reader();
    ...
    std::cout << "[bbggNonResMVA::SetupNonResMVA] NonResMVA set with the following variables (attention, the order is important): " << std::endl;
...
//      LowMassReader->AddVariable(it->first, mvaVars[it->first]);
//      HighMassReader->AddVariable(it->first, mvaVars[it->first]);
      LowMassReader->AddVariable(it->first, &it->second);
      HighMassReader->AddVariable(it->first, &it->second);
      std::cout << "\t" << it->first << std::endl;
    }
    LowMassReader->BookMVA("BDT", LMFile); 
    HighMassReader->BookMVA("BDT", HMFile); 

}

double bbggNonResMVA::DeltaR(bbggNonResMVA::LorentzVector vec1, bbggNonResMVA::LorentzVector vec2)
{
...
std::vector<float> bbggNonResMVA::mvaDiscriminants(std::map<std::string,float> params)
{
  ...
  std::vector<float> mvaDis;
   ...
   float LMdis = LowMassReader->EvaluateMVA("BDT");
   float HMdis = HighMassReader->EvaluateMVA("BDT");

   mvaDis.push_back(LMdis);
   mvaDis.push_back(HMdis);
